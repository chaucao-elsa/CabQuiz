...
const rotateQuestionsAndCalculateScores = async (context) => {
  ...
  for (const roomDoc of roomsSnapshot.docs) {
    ...
    const randomQuestionId = `question_${Math.floor(
      Math.random() * questionCount
    )}`;

    // Get the random question
    const randomQuestionDoc = await questionsRef.doc(randomQuestionId).get();
    const randomQuestion = {
      id: randomQuestionDoc.id,
      ...randomQuestionDoc.data(),
    };
    ...
    participantsSnapshot.forEach((participantDoc) => {
      ...
      if (participantData.answer_time !== undefined) {
        ...
        if (
          participantData.last_answer ===
          roomData.current_question.correct_option
        ) {
          // Calculate time taken to answer
          const timeTaken = Math.min(
            (participantData.answer_time.toMillis() -
              roomData.start_time.toMillis()) /
              1000,
            TOTAL_TIME
          );
          ...
          let score =
            MAX_SCORE - ((MAX_SCORE - MIN_SCORE) * timeTaken) / TOTAL_TIME;
          ...
          console.log(
            `User ${userId} in room ${roomId} earned ${score} points!`
          );
        } else {
    ...
    console.log(
      `Room ${roomId} rotated to question: ${randomQuestion.question_text}`
    );
  }
